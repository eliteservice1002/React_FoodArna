...
import '../../../partPage/commonStyle.css';
...
function OrderStatus(props) {
    ...
    const init = () => {
        ...
        var itemarr = [];
        var settings = {
            "url": ApiConfig.url + "/api/orders/chef/" + localState.data.chefId,
            "method": "GET",
            "timeout": 0,
            "headers": {
    ...
    const Readmore = (e) => {
        ...
        $.ajax(settings).done(async function (response) {
            ...
            setTimeout(function () {
                document.getElementById('btn_backdrop').click();
                history.push('/orderstate', { flag: "chef" });
            }, 2000)
        });
    }
                        ...
                        <Grid item xs={12} sm={12} style={{ display: 'flex', flexWrap: 'wrap' }}>
                            <Grid item xs={12} sm={12} style={{ height: '12.628%', backgroundColor: '#F8C545', }}>
                                <Grid item xs={4} sm={4} style={{ marginTop: '1%' }}>
                                    <img src={"./images/partPage/chef_order_state.png"} style={{ cursor: 'pointer', marginLeft: '1.5%' }} alt="alt" onClick={() => { history.push("/") }}></img>
                                </Grid>
                            </Grid>
                            ...
                            <Grid item xs={12} sm={12} style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', margin: 10 }}>
                                ...
                                <Button variant="outlined" className="item-add-button" onClick={() => { history.push('/sellMenu') }} style={{ width: 128, height: 40, borderRadius: '25px', color: '#fff' }}>Back</Button>
                            </Grid>
                        </Grid>
                    </Grid>
                </Grid>
